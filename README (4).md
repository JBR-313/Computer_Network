...
> **My Contribution:** Dataset construction (`make_csv.py`) · Visualization pipeline (`visualize.py`) · Insight extraction from results

---

## My Role: Data Visualization & Insight Extraction

In this project, I was responsible for **constructing the experimental dataset, building the visualization pipeline, and drawing analytical insights from the results**. While other members focused on porting cryptographic algorithms and running Cooja simulations, my job was to turn raw measurement numbers into graphs that could tell a clear story — and to ask the right questions about what those graphs actually meant.
...
```python
# make_csv.py — constructs the measurement CSV from raw simulation values
import pandas as pd

data = {
    'algorithm': ['ASCON', 'ASCON', 'AES-GCM', 'AES-GCM'],
    'data_size': [16, 64, 16, 64],
    'cpu_ticks': [3.271, 4.451, 15.22, 59.14],
    'energy_mJ': [0.00431, 0.00587, 0.00250, 0.00974]
}

df = pd.DataFrame(data)
df.to_csv('energy_simulation_results.csv', index=False)
```

### Step 2 — Generate all three charts

```python
# visualize.py — core visualization logic (excerpt)
df['time_s'] = df['cpu_ticks'] / 4096
df['energy_mJ'] = 1.8 * 3 * df['time_s']
# Energy derived from Sky mote power model: P = 1.8mA × 3V = 5.4mW

# Line chart: reveals the crossover trajectory between algorithms
ascon  = df[df['algorithm'] == 'ASCON']
aesgcm = df[df['algorithm'] == 'AES-GCM']

fig, ax = plt.subplots(figsize=(8, 5))
ax.plot([16, 64], ascon['energy_mJ'].values,  marker='o', label='ASCON')
ax.plot([16, 64], aesgcm['energy_mJ'].values, marker='s', label='AES-GCM')
ax.set_xlabel('Data Size (bytes)')
ax.set_ylabel('Energy (mJ)')
ax.legend()
plt.savefig('results/energy_comparison.png')

# Bar chart: absolute energy side-by-side at each payload size
# Avg+Std chart: measurement stability across 1,000 runs
```
